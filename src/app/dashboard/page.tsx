...
export default function DashboardClient() {
  ...
  React.useEffect(() => {
    const fetchUserProfile = async () => {
      try {
        ...
        if (profile) {
          // Format the user info from the profile
          const formattedUserInfo: UserInfo = {
            id: profile.id, // Add ID if available
          ...
          saveUserInfo(formattedUserInfo);
  ...
  return (
    <DashboardLayout userProfile={userInfo} isOffline={isOffline}>
      <Box sx={{ flex: 1 }}>
